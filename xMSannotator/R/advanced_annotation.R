.get_peak_blocks_modulesvhclust <- function(data, time_step, max_rt_diff, deep_split, min_cluster_size, cutheight, cormethod, networktype = "unsigned", workers, mycl_metabs = NA) {
  ...
  power_val <- tryCatch(WGCNA::pickSoftThreshold(data = data_m, dataIsExpr = TRUE, powerVector = c(1:10, seq(from = 12, to = 20, by = 2)), verbose = 0)$powerEstimate, error = function(...) NA)
  if (is.na(power_val)) {
    power_val <- 6
  }
...
.clusterring <- function(data, correlation_method, correlation_threshold, min_cluster_size, deep_split, network_type, workers) {
  ...
  global_correlation <- WGCNA::cor(t(data[, -(1:2)]), nThreads = workers, method = correlation_method, use = 'p')
  colnames(global_correlation) <- rownames(global_correlation) <- paste(data$mz, data$rt, sep = '_')

  hr <- flashClust::flashClust(as.dist(1 - global_correlation), method = 'complete')
  mycl_metabs <- dynamicTreeCut::cutreeDynamic(hr, distM = 1 - global_correlation, deepSplit = 1, minClusterSize = min_cluster_size, pamRespectsDendro = FALSE, pamStage = TRUE, verbose = 0)
  ...
  clustering <- clustering[order(clustering$mz, clustering$rt),]

  return(clustering)
}

.step5 <- function(result, adduct_weights, score_threshold = 0) {
  result <- result[order(result$confidence, result$chemical_id, result$score, result$adduct, decreasing = TRUE)]
...
advanced_annotation <- function(data, metabolites_db, adduct_weights, max_mz_diff, max_rt_diff, correlation_method, correlation_threshold, min_cluster_size, deep_split, network_type, boost_metabolites, expected_adducts, min_isp, max_isp, strict_boosting, redundancy_filtering, workers = future::availableCores()) {
  data <- unique(as.data.frame(data), by = c('mz', 'rt'))
  metabolites_db <- as.data.frame(metabolites_db)
  ...
  multilevelannotation(
    ...
    max_isp = max_isp, customDB = metabolites_db, pathwaycheckmode = if (strict_boosting) 'pm' else 'p'
  )
