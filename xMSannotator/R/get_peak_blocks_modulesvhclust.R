get_peak_blocks_modulesvhclust <- function(dataA = NA, simmat = NA, adjacencyfromsimilarity = FALSE, time_step = 3, max.rt.diff = 10, outloc, column.rm.index = NA, cor.thresh = NA, deepsplit = 2, minclustsize = 20, cutheight = 0.2, cormethod = "spearman", networktype = "unsigned", num_nodes = 2, step1log2scale = TRUE, mycl_metabs = NA) {
  ...
  if (do_stepwise == 1) {
    ...
    if (length(dup_names) > 0) {
      ADJdataOne <- ADJdataOne[-dup_names, -dup_names]
    }
