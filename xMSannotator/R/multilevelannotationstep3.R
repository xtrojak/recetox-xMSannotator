multilevelannotationstep3 <- function(outloc1, adduct_weights, num_sets = NA, boostIDs = NA, pathwaycheckmode = "p", dbAllinf = NA, scorethresh = 0.1) {
  setwd(outloc1)
  ...
  chemscoremat <- lapply(1:num_sets, function(sind) {
    cur_fname <- paste0("chem_score", sind, ".Rda")
  ...
  chemscoremat <- plyr::ldply(chemscoremat, rbind)
          ...
          if (p1 > pthresh) {
            ...
            for (c in pathway_chemicals_1) {
              ...
              if (nrow(curmchemicaldata) > 0) {
                ...
                cur_module_data <- chemscoremat[which(chemscoremat$module_num == cur_module &
                ...
                cur_module_data2 <- chemscoremat[which(chemscoremat$module_num == cur_module &
                                                         chemscoremat$score >= scorethresh &
                                                         chemscoremat$Adduct %in% as.character(adduct_weights[, 1])),]
                b <- length(unique(cur_module_data2$chemical_ID)) - a
                rm(cur_module_data2)

                other_module_data <- chemscoremat[which(chemscoremat$module_num != cur_module &
                                                          chemscoremat$chemical_ID %in% pathway_chemicals &
                                                          chemscoremat$score >= scorethresh &
                                                          chemscoremat$Adduct %in% as.character(adduct_weights[, 1])),]
                ...
                other_module_data2 <- chemscoremat[which(chemscoremat$module_num != cur_module &
                                                           chemscoremat$score >= scorethresh &
                                                           chemscoremat$Adduct %in% as.character(adduct_weights[, 1])),]
