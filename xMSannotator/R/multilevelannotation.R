multilevelannotation <- function(dataA, max.mz.diff = 10, max.rt.diff = 10, cormethod = "pearson", num_nodes = 2, queryadductlist = c("all"), gradienttype = "Acetonitrile", mode = "pos", outloc, db_name = "HMDB", adduct_weights = NA, num_sets = 3000, allsteps = TRUE, corthresh = 0.7, NOPS_check = TRUE, customIDs = NA, missing.value = NA, deepsplit = 2, 
    networktype = "unsigned", minclustsize = 10, module.merge.dissimilarity = 0.2, filter.by = c("M+H"), redundancy_check = TRUE, min_ions_perchem = 1, biofluid.location = NA, origin = NA, status = NA, boostIDs = NA, max_isp = 5, MplusH.abundance.ratio.check = FALSE, customDB = NA, HMDBselect = "union", mass_defect_window = 0.01, mass_defect_mode = "pos", 
    ...
    # for(db_index in 1:length(db_name_list))
    {
        ...
        if (length(check_step1) < 1) {
            ...
            if (length(check_levelA) < 1) {
                if (clustmethod == "WGCNA") {
                  levelA_res <- get_peak_blocks_modulesvhclust(dataA = dataA, simmat = NA, adjacencyfromsimilarity = FALSE, time_step = time_step, max.rt.diff = max_diff_rt, outloc, column.rm.index = NA, cor.thresh = NA, deepsplit = deepsplit, minclustsize = minclustsize, cutheight = cutheight, cormethod = cormethod, networktype = networktype,
                    num_nodes = num_nodes, step1log2scale = step1log2scale, mycl_metabs = mycl_metabs)
                ...
                # alldegrees<-levelA_res[,c(1:4)] levelA_res<-levelA_res[,-c(1:4)]
            }
            ...
            if (db_name == "HMDB") {
                ...
                # if(length(which(hmdbCompMZ$HMDBID%in%hmdbbad))>0){ hmdbCompMZ<-hmdbCompMZ[-which(hmdbCompMZ$HMDBID%in%hmdbbad),]
                # }
            ...
            if (length(check_levelB) < 1) {
                ...
                # formula_ID<-paste('Formula',seq(1:dim(chemCompMZ_unique_formulas)[1]),sep='_') chemCompMZ_unique_formulas$chemical_ID<-formula_ID
                ...
                # annotation_multilevel_ save(list=ls(),file='test.Rda') system.time(l2<-parLapply(cl,s1,Annotationbychemical_IDschild_multilevel,dataA=dataA, queryadductlist=c(adduct_names),adduct_type=c('S',gradienttype),
                ...
                # a1<-Annotationbychemical_IDschild_multilevel(1,dataA=dataA, queryadductlist=c(adduct_names),adduct_type=c('S',gradienttype), adduct_table=adduct_table,max.mz.diff=max.mz.diff,outloc=outloc,keggCompMZ=chemCompMZ_unique_formulas,otherdbs=FALSE,otherinfo=FALSE,chemIDs=chemIDs,num_nodes=num_nodes)
                # a1<-Annotationbychemical_IDschild(1,dataA=dataA, queryadductlist=c(adduct_names),adduct_type=c('S',gradienttype), adduct_table=adduct_table,max.mz.diff=max.mz.diff,outloc=outloc,keggCompMZ=chemCompMZ_unique_formulas,otherdbs=FALSE,otherinfo=FALSE,num_nodes=num_nodes)

                l2 <- parLapply(cl, s1, Annotationbychemical_IDschild, dataA = dataA, queryadductlist = c(adduct_names), adduct_type = c("S", gradienttype), adduct_table = adduct_table, max.mz.diff = max.mz.diff, outloc = outloc, keggCompMZ = chemCompMZ_unique_formulas, otherdbs = FALSE, otherinfo = FALSE, num_nodes = num_nodes)
                
                stopCluster(cl)
                
                # print(format(object.size(l2),unit='auto')) save(l2,file='l2.Rda')
            ...
            save(list = c("mchemdata", "chemids", "adduct_table", "mzid", "max_diff_rt", "isop_res_md", "corthresh", "level_module_isop_annot", "chemids_split", "corthresh", "max.mz.diff", "outloc", "num_sets", "db_name", "num_nodes", "num_sets", "adduct_weights", "filter.by", "max.rt.diff", "max_isp", "MplusH.abundance.ratio.check", "mass_defect_window",
                "mass_defect_mode", "allsteps"), file = "step1_results.Rda")
        ...
        if (allsteps == TRUE) {
            ...
            if (num_sets > num_nodes) {
                ...
                clusterEvalQ(cl, "library(plyr)")
                # clusterEvalQ(cl, 'library(pryr)') clusterEvalQ(cl, 'library(profmem)') clusterEvalQ(cl, 'library(gdata)')
            ...
            # print('Memory used after step2 before removing all objects') print(mem_used())
            rm(list = ls())
            try(rm(hmdbCompMZ), silent = TRUE)
            
            load("tempobjects.Rda")
            
            # print('Memory used after step2 and removing all objects') print(mem_used()) print(ll()[order(ll()$KB),])
            # if(allsteps==TRUE)
            {
