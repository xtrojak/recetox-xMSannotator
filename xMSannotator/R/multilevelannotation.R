multilevelannotation <- function(dataA, max.mz.diff = 10, max.rt.diff = 10, cormethod = "pearson", num_nodes = parallel::detectCores(), queryadductlist = "all", gradienttype = "Acetonitrile", mode = "pos", outloc, db_name = "HMDB", adduct_table, adduct_weights, num_sets = 3000, allsteps = TRUE,
  ...
  if (length(check_step1) < 1) {
    ...
    if (db_name == "HMDB") {
      ...
      mz_search_list <- lapply(seq_len(nrow(inputmassmat)), function(m) {
        xMSannotator::get_mz_by_monoisotopicmass(monoisotopicmass = as.numeric(as.character(inputmassmat[m, 4])), dbid = inputmassmat[m, 1], name = as.character(inputmassmat[m, 2]), formula = as.character(inputmassmat[m, 3]), queryadductlist = as.character(adduct_names), adduct_table = adduct_table)
      })
    ...
    if (length(check_levelB) < 1) {
      ...
      l2 <- parallel::parLapply(cl, s1, Annotationbychemical_IDschild, dataA = dataA, queryadductlist = adduct_names, adduct_type = c("S", gradienttype), adduct_table = adduct_table, max.mz.diff = max.mz.diff, outloc = outloc, keggCompMZ = chemCompMZ_unique_formulas, otherdbs = FALSE, otherinfo = FALSE, num_nodes = 1)
  ...
  if (allsteps == TRUE) {
    ...
    if (num_sets > num_nodes) {
      ...
      parallel::parLapply(cl, 1:num_sets, function(arg1) {
        ...
        if (is(check_if_exists, "try-error")) {
          xMSannotator::multilevelannotationstep2(outloc1 = outloc, list_number = arg1, adduct_table = adduct_table, adduct_weights = adduct_weights)
        } else {
    ...
    multilevelannotationstep4(outloc = outloc, max.rt.diff = max_diff_rt, filter.by = filter.by, adduct_table = adduct_table, adduct_weights = adduct_weights, min_ions_perchem = min_ions_perchem, boostIDs = boostIDs, max_isp = max_isp, max.mz.diff = max.mz.diff)
    ...
    if (redundancy_check == TRUE) {
      ...
      suppressWarnings(multilevelannotationstep5(outloc = outloc, max.rt.diff = max_diff_rt, filter.by = filter.by, adduct_table = adduct_table, adduct_weights = adduct_weights, min_ions_perchem = min_ions_perchem, boostIDs = boostIDs, max_isp = max_isp, db_name = db_name, max.mz.diff = max.mz.diff))
    }
  }
