get_confidence_stage4 <- function(curdata, max_diff_rt, adduct_table, adduct_weights, filter.by = NA, min_ions_perchem = 1) {
  curdata <- curdata[order(curdata$Adduct),]

  if (curdata$score[1] < 0.1) {
    return(cbind(0, curdata))
  }
  ...
  if (length(unique(curdata$Adduct)) < 2 &&
    curdata$score < 10 &&
    length(which(cur_adducts %in% filter.by)) < 1) {
    chemscoremat_conf_levels <- "None"
  } else {
  ...
  if (delta_rt > max_diff_rt) {
    ...
    module_clust <- paste0(curdata$module_num, curdata$Module_RTclust)
    curdata$Module_RTclust <- module_clust
  }
  curdata <- as.data.frame(curdata)
