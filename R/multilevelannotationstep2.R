...
replace_with_module <- function(module_rt_clust) {
    gsub(module_rt_clust, pattern = "_[0-9]*", replacement = "")
}
...
compute_chemscore <- function(j,
    ...
    isp_masses_mz_data <- {}
    isp_masses_mz_data <- lapply(1:length(curmchemdata$mz), compute_something, curmchemdata, mass_defect_window, isop_res_md)
...
multilevelannotationstep2 <-
    function(outloc1, list_number) {
        ...
        rm(
            "curchemscoremat", "mchemdata", "chemids", "adduct_table", "global_cor", "mzid", "max_diff_rt", "isop_res_md", "corthresh", "level_module_isop_annot",
