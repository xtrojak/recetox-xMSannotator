...
Annotationbychemical_IDschild <-
    function(adduct_index = NA, dataA, queryadductlist = c("M+H"), adduct_type = c("S", "Acetonitrile"), adduct_table, max.mz.diff = 10, outloc, otherdbs = FALSE, otherinfo = FALSE, keggCompMZ, num_nodes = 2) {
        ...
        gcur <- getVenn(
            dataA = dataA, name_a = "Experimental", name_b = "DB", dataB = mz_search_list_1, mz.thresh = max.mz.diff, time.thresh = NA,
                        ...
                        if (length(t2) > 1) {
                            ...
                            if (length(g_ind) > 0) {
                                if (length(g_ind) > 1) {
                                    ...
                                    lipidmapsID <- gsub(b1[g_ind, 2], pattern = "lipidmaps:", replacement = "")
                                }
                            }
                        }
