multilevelannotationstep3 <- function(outloc1,
  ...
  if (is.na(chemscoremat)) {
    ...
    chemscoremat <- lapply(1:num_sets, function(sind)
    {
      ...
      curchemscoremat$Formula <-
        gsub(curchemscoremat$Formula,
             pattern = "_.*",
             replacement = "")
    ...
    chemscoremat$Formula <-
      gsub(chemscoremat$Formula,
           pattern = "_.*",
           replacement = "")
    chemscoremat <- as.data.frame(chemscoremat)
  }
  ...
  chemscoremat <-
    merge(chemscoremat, chemCompMZ[, c(2:4, 6)], by = c("Formula", "Adduct"))
  ...
  if (length(which(chemscoremat$chemical_ID %in% hmdbbad)) > 0) {
    chemscoremat <-
      chemscoremat[-which(chemscoremat$chemical_ID %in% hmdbbad), ]
  }
                    ...
                    if (num_chems < 3) {
                      ...
                      if (curmchemicaldata$score[1] < scorethresh) {
                        ...
                        chemscoremat$score[which(as.character(chemscoremat$chemical_ID) == c)] =
                          chemscoremat$score[which(as.character(chemscoremat$chemical_ID) == c)][1] +
                          num_chems
