test_that("Isotopes-computing: isotopes annotation.", {
  ...
  actual <- compute_isotopes(annotation = annotation,
...
patrick::with_parameters_test_that("Isotopes-computing: isotopes detection (single annotation).", {
  peaks <- readRDS("test-data/isotopes_detection/peaks.rds")
  actual <- detect_isotopic_peaks(peaks = peaks,
                             intensity_deviation_tolerance = 0.25,
                             rt_tolerance = 10,
                             mass_defect_tolerance = 0.1,
                             query)
...
patrick::with_parameters_test_that("Isotopes-computing: theoretical isotopic pattern", {
